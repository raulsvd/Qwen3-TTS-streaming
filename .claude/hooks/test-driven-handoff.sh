...
agent_tdd_checkpoint() {
    ...
    while [[ "$current_dir" != "/" ]]; do
        if [[ -d "$current_dir/.claude-collective" ]]; then
    ...
    if [[ -z "$project_root" ]]; then
        log "❌ AGENT TDD FAILURE: $agent_name - could not find .claude-collective directory"
        return 1
    fi

    # Check for dependencies first to avoid false positives
    if [[ ! -d "$project_root/.claude-collective/node_modules" ]]; then
        log "Installing dependencies in $project_root/.claude-collective/ for testing..."
    ...
    log "🧪 Running vitest validation for $agent_name from $project_root/.claude-collective..."
